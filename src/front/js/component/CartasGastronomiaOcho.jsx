...
const CartasGastronomiaOcho = () => {
  ...
  useEffect(() => {
    ...
    const loadGastronomia = async () => {
      try {
        await actions.cargarServiciosGastronomia();
        if (isMounted) setLoading(false);
      } catch (err) {
        console.error("Error al cargar los servicios de gastronomía:", err);
        if (isMounted) {
      ...
      <section className="py-5">
        <div className="col-12 col-sm-10 col-md-9 col-lg-9 mx-auto px-3 px-sm-4">
          <div className="text-center">
            ...
            <p className="mt-2">Cargando servicios de gastronomía...</p>
          </div>
        </div>
      </section>
    );
  ...
  const gastronomiaAMostrar = showAll ? store.serviciosGastronomia : store.serviciosGastronomia.slice(0, 8);

  return (
    <section className="py-5">
      <div className="col-12 col-sm-10 col-md-9 col-lg-9 mx-auto px-3 px-sm-4">
        <div className="d-flex justify-content-between align-items-center mb-4">
          <h2 className="fw-bold fs-4 mb-0">Ofertas de Gastronomía</h2>
          <div>
            <button 
        ...
        {store.serviciosGastronomia.length === 0 && (
          <div className="alert alert-info mt-4">
            No hay servicios de gastronomía disponibles actualmente.
          </div>
        )}
      </div>
    </section>
